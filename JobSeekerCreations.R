...
# Read data
df <- read_excel('Downloads/Group Project Option 2 and 3 HHUSA/EPO_Teradata_Job Seeker_Profile_Creation_Report.xlsx')
...
dftime$created_at_year <- year(dftime$created_at)
dftime.grp.year <- group_by(dftime,created_at_year)
tally(dftime.grp.year)
year_created <- factor(dftime$created_at_year)
ggplot(dftime,aes(x=year_created))+geom_bar()

dftime2018 <- dftime[-c(3881:4342),]
summary(dftime2018)
